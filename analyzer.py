...
from modeling import (
    ...
    XLNetForSentimentClassification,
)
...
class Analyzer:
    def __init__(self, will_train, args):
        ...
        if self.config.model_type == "bert":
            ...
            self.model = XLNetForSentimentClassification.from_pretrained(
                args.model_name_or_path
            )
        else:
    ...
    def evaluate(self, val_loader, criterion):
        ...
        batch_accuracy_summation, loss, num_batches, precision, recall = 0, 0, 0, 0, 0
        ...
        with torch.no_grad():
            ...
            for input_ids, attention_mask, labels in tqdm(
                ...
                acc, pre, rec = get_accuracy_from_logits(logits, labels)
                probabilties = torch.sigmoid(logits.unsqueeze(-1))
                preds.append(probabilties.tolist())
                batch_accuracy_summation += acc
                precision += pre
                recall += rec
    ...
    def classify_sentiment(self, text):
        ...
        with torch.no_grad():
            ...
            if is_positive:
                return "Suicide", int(positive_percentage)
            else:
                return "Non-Suicide", int(100 - positive_percentage)
